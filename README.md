...
## Architecture Changes — June 2026 (Astro migration)

This section documents what changed in the June 19–24 2026 sprint so the next agent starts with full context.

### What changed

**The presentation layer moved from a generated HTML file to an Astro v7 static build.**

Before this sprint, `dashboard.html` (a single 2000+ line file generated by the pipeline) was the entire frontend. After this sprint, the frontend is an Astro project in `src/`. The build outputs to `dist/`, which is what both `server.py` and Vercel serve. `dashboard.html` still exists as a legacy pipeline artifact but is no longer the canonical UI.
...
| `scripts/future-caribbean-holding.html` | Self-contained 404/holding page for `future-caribbean.vercel.app` |
| `fallow.json` | Fallow dead-code gate config — suppresses false positives on Astro deps |

**`server.py` routing** was updated to serve from `dist/` instead of the legacy files:
...
This is handled by `scripts/vercel-build.sh` which reads the `VERCEL_PROJECT_NAME` env var Vercel injects at build time:
...
`vercel.json` uses the **new Vercel format** — `buildCommand` + `outputDirectory`, **no `builds` array**. The legacy `builds` array format silently ignored `buildCommand` and read from git source. Do not add a `builds` key back — it will break the pipeline invisibly.

### Vercel route order matters
...
The `/signal/(.*) → /signal/$1/index.html` route is required. The generic catchall resolves to a directory path, not the `index.html` inside it — Astro's static output puts each signal page at `dist/signal/DSP-xxx/index.html`.

### Signal permalink IDs are cycle-dated

Signal IDs: `DSP-YYYYMMDD-NNN`. They are generated fresh each pipeline cycle. After a new run, old IDs 404 cleanly — no redirect or tombstone exists. This is by design. Current cycle: `DSP-20260624-001` through `DSP-20260624-032`.
...
`dist/` is gitignored. Vercel builds it remotely. If you delete it locally, run `pnpm build`.

---

### Wins

- **Astro build works end-to-end** — 34 pages in ~450ms, signal permalink pages resolve on Vercel
- **Dual Vercel project routing** via `VERCEL_PROJECT_NAME` — zero Vercel dashboard config needed, one build script handles both
- **Interactive Leaflet map restored** — SRI `integrity` hash attrs on the CDN `<script>` and `<link>` tags caused the browser to silently block Leaflet before making any network request; removing them (matching how React/htm are loaded) fixed it
- **Fallow gate still active** — pre-commit dead-code scanner passes via `fallow.json` + targeted `fallow-ignore` comments; no bypass
- **`server.py` and Vercel now serve the same source** — local and prod both read from `dist/`

### Fails / known friction

- **Vercel build cache lies** — if you push a fix and the deployment still shows old behaviour, run `vercel deploy --prod --force` from the repo root to bust it. Normal git-push redeploys can serve stale static output even after route changes
- **Legacy `builds` array is a silent trap** — adding `"builds": [...]` back to `vercel.json` will cause `buildCommand` to be ignored and break deployments with no error message
- **`dashboard.html` is now a zombie** — still committed, still generated by the pipeline, still served at `/dashboard.html` via `server.py`, but the canonical UI is `dist/index.html`. The two can diverge. `dashboard/generate.py` should eventually be retired or replaced by writing data into Astro's data layer
- **`pnpm dev` has no API** — Astro's HMR server (port 4321) does not proxy `/api/*` to `server.py`. For full API + frontend testing use `pnpm build && python3 server.py`
- **Signal IDs rotate each cycle** — any bookmarked `/signal/DSP-xxx` URL goes 404 after the next pipeline run. No redirect or ID-persistence strategy exists yet
