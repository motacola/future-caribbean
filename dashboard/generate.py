...
# ── Live Wire ticker (same selection as the RSS feed) ──────
_seen_ticker: dict[str, dict] = {}
for _d in dispatches:
    _sid = _d.get("signal_id") or _d.get("dispatch_id", "")
    _cur = _seen_ticker.get(_sid)
    if _cur is None or (_d.get("confidence_score", 0) or 0) > (_cur.get("confidence_score", 0) or 0):
        _seen_ticker[_sid] = _d
_ticker_items = sorted(_seen_ticker.values(), key=lambda d: -(d.get("confidence_score", 0) or 0))[:12]

KIND_COLORS = {"enhanced_investment": "#2676A8", "investment_signal": "#2676A8",
               "climate": "#0D766E", "weather": "#0D766E",
               "procurement": "#B57A22", "development_pipeline": "#B57A22"}
ticker_html = ""
for _t in _ticker_items:
    _kind = _t.get("signal_kind", "")
    _color = next((v for k, v in KIND_COLORS.items() if k in _kind), "#7F8C83")
    _country = canonical_country(_t.get("country_cluster", "")) or ""
    ticker_html += (
        f'<a class="lw-item" href="#leaflet-map" data-country="{j(_country)}">'
        f'<i style="background:{_color}"></i>'
        f'<span class="lw-kicker">{j(_t.get("country_cluster", "Region"))}</span>'
