...
**Base URL (hosted):** `https://future-caribbean.fly.dev`
...
curl http://localhost:8080/api/validation-packs

# Get a specific pack
curl http://localhost:8080/api/validation-packs/enhanced-invest-guyana

# Tool manifest (for any tool-calling agent)
curl http://localhost:8080/api/tools.json
```
...
```json
{
  "name": "caribbean-desk",
  "type": "http",
  "url": "http://localhost:8080/api/tools.json",
  "auth": "none"
}
```
...
```bash
hermes tools add http caribbean-desk http://localhost:8080/api/tools.json
```
...
OpenClaw's HTTP tools pointed at `http://localhost:8080/api/tools.json`.
